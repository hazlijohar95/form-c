...
export function WordmarkV2(props: Pick<SVGProps<SVGSVGElement>, "className" | "style">) {
  const rawId = useId();
  const safeId = rawId.replace(/[^a-zA-Z0-9]/g, "");
  const mask = `wordmark-mask-${safeId}`;
  const maskGradient = `wordmark-gradient-${safeId}`;
